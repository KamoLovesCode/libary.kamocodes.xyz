...
const Dashboard: React.FC<DashboardProps> = ({ currentUser, onLogout }) => {
  ...
  const renderNavbar = () => {
    ...
    return (
      <div className="pill-navbar-container" style={{ bottom: '24px', width: '100%', padding: '0 24px', maxWidth: '500px' }}>
        {showNotifications && (
            <div className="notification-popup" style={{ background: 'var(--bg-surface)', borderRadius: '24px', padding: '20px', marginBottom: '20px', border: '1px solid rgba(255,255,255,0.05)', boxShadow: '0 20px 40px rgba(0,0,0,0.5)' }}>
                <h4 className="text-title" style={{fontSize: '1rem', marginBottom: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                    AI Suggestions
                    <button onClick={() => setShowNotifications(false)} className="btn-icon" style={{ width: '32px', height: '32px', padding: 0, background: 'rgba(255,255,255,0.05)' }}><Icon name="close" style={{fontSize: '18px'}}/></button>
                </h4>
                {notifications.length === 0 ? (
                    <p className="text-caption" style={{ fontSize: '0.95rem' }}>No new suggestions.</p>
                ) : (
                    <ul style={{listStyle: 'none', maxHeight: '250px', overflowY: 'auto', margin: 0, padding: 0}}>
                        {notifications.map(notif => (
                            <li key={notif.id} style={{marginBottom: '16px', paddingBottom: '16px', borderBottom: '1px solid rgba(255,255,255,0.05)'}}>
                                <p className="text-body" style={{fontSize: '0.95rem', color: 'var(--text-secondary)', lineHeight: 1.5}}>{notif.text}</p>
                            </li>
                        ))}
                    </ul>
                )}
        ...
        <nav style={{ 
          ...
          <button className={`nav-pill-item nav-icon-only ${currentView === 'HOME' ? 'active' : ''}`} onClick={() => handleNav('HOME')} style={{ padding: '12px', background: 'transparent' }}>
            <Icon name="home" style={{ fontSize: '28px', color: currentView === 'HOME' ? 'var(--accent-color)' : 'var(--text-secondary)', transition: 'color 0.2s' }} />
          </button>
          <button className={`nav-pill-item nav-icon-only ${currentView === 'LIBRARY' ? 'active' : ''}`} onClick={() => handleNav('LIBRARY')} style={{ padding: '12px', background: 'transparent' }}>
            <Icon name="grid_view" style={{ fontSize: '28px', color: currentView === 'LIBRARY' ? 'var(--accent-color)' : 'var(--text-secondary)', transition: 'color 0.2s' }} />
          </button>
          ...
          <div style={{ position: 'relative', top: '-28px' }}>
              <button 
                onClick={() => {
                    handleNav('HOME');
                    setTimeout(() => (document.querySelector('.quick-paste-area') as HTMLTextAreaElement)?.focus(), 100);
                }} 
                style={{ 
                    ...
                    boxShadow: '0 12px 28px rgba(var(--accent-rgb), 0.4)',
                    cursor: 'pointer',
              ...
              >
                <Icon name="add" style={{ fontSize: '36px' }} />
              </button>
          </div>

          <button className={`nav-pill-item nav-icon-only ${showNotifications ? 'active' : ''}`} onClick={() => setShowNotifications(!showNotifications)} style={{ padding: '12px', background: 'transparent' }}>
             <Icon name={notifications.length > 0 ? "notifications_active" : "notifications"} style={{ fontSize: '28px', color: showNotifications ? 'var(--accent-color)' : 'var(--text-secondary)', transition: 'color 0.2s' }} />
          </button>
          <button className={`nav-pill-item nav-icon-only ${currentView === 'PROFILE' ? 'active' : ''}`} onClick={() => handleNav('PROFILE')} style={{ padding: '12px', background: 'transparent' }}>
             <Icon name="person" style={{ fontSize: '28px', color: currentView === 'PROFILE' ? 'var(--accent-color)' : 'var(--text-secondary)', transition: 'color 0.2s' }} />
          </button>
        </nav>
      </div>
    );
  ...
  const renderHome = () => {
    ...
    return (
      <div className="page-container quick-paste-wrapper" style={{ padding: '20px 20px 140px 20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px', marginTop: '20px' }}>
            <div>
                <p style={{ fontSize: '1rem', color: 'var(--text-secondary)', marginBottom: '4px' }}>Hello {currentUser.username} 👋</p>
                <h1 style={{ fontSize: '2.2rem', fontWeight: 700, color: 'var(--text-main)', lineHeight: 1.2, letterSpacing: '-0.03em' }}>Capture Your<br/>Thoughts</h1>
            </div>
            <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: 'var(--bg-surface)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid rgba(255,255,255,0.05)' }}>
                <Icon name="notifications" style={{ color: 'var(--text-secondary)', fontSize: '24px' }} />
            </div>
        </div>
        
        <div className="card" style={{ position: 'relative', width: '100%', marginBottom: '40px', borderRadius: '28px', padding: '4px', background: 'linear-gradient(145deg, var(--bg-surface), var(--bg-app))', border: '1px solid rgba(255,255,255,0.05)', boxShadow: '0 20px 40px rgba(0,0,0,0.2)' }}>
            <textarea 
                ...
                style={{ minHeight: '160px', padding: '24px', paddingBottom: '80px', background: 'transparent', border: 'none', boxShadow: 'none', fontSize: '1.1rem', resize: 'none', color: 'var(--text-main)' }}
            />
            <div style={{ position: 'absolute', bottom: '20px', right: '20px', display: 'flex', gap: '12px', alignItems: 'center' }}>
                <button onClick={handleSummarize} className="btn-icon" style={{ background: 'rgba(255,255,255,0.05)', width: '44px', height: '44px', borderRadius: '50%', color: 'var(--text-secondary)' }}>
                    <Icon name={isSummarizing ? "autorenew" : "auto_awesome"} style={{ fontSize: '22px' }} />
                </button>
                <button onClick={handleQuickSave} className="btn btn-primary" style={{ padding: '12px 24px', borderRadius: '100px', fontSize: '1rem', fontWeight: 600, boxShadow: '0 8px 20px rgba(var(--accent-rgb), 0.3)' }} disabled={!quickPasteContent.trim()}>
                    Save Note
                </button>
            </div>
        </div>
        ...
        <div style={{ width: '100%' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                <h2 className="text-title" style={{ fontSize: '1.4rem', fontWeight: 600, letterSpacing: '-0.02em' }}>Recent Notes</h2>
                {sorted.length > 5 && (
                    <span onClick={() => handleNav('LIBRARY')} style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', cursor: 'pointer', fontWeight: 500 }}>See All</span>
                )}
            </div>
            
            {sorted.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '4rem 2rem', opacity: 0.5, background: 'var(--bg-surface)', borderRadius: '28px', border: '1px dashed var(--border)' }}>
                    <Icon name="edit_note" style={{ fontSize: '3.5rem', marginBottom: '1rem', color: 'var(--text-secondary)' }} />
                    <p style={{ fontSize: '1.1rem' }}>No notes yet. Start typing above.</p>
                </div>
            ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    {sorted.slice(0, 5).map(entry => (
                        <div key={entry.id} className="card" onClick={() => handleSelectEntry(entry)} style={{cursor: 'pointer', padding: '24px', display: 'flex', flexDirection: 'column', gap: '14px', borderRadius: '24px', background: 'var(--bg-surface)', border: '1px solid rgba(255,255,255,0.03)', transition: 'transform 0.2s, background 0.2s'}} onMouseEnter={e => e.currentTarget.style.transform = 'scale(1.02)'} onMouseLeave={e => e.currentTarget.style.transform = 'scale(1)'}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <span style={{ background: 'rgba(var(--accent-rgb), 0.15)', color: 'var(--accent-color)', padding: '6px 12px', borderRadius: '10px', fontSize: '0.8rem', fontWeight: 600 }}>Note</span>
                                <span className="text-caption" style={{ fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: '6px' }}>
                                    <Icon name="schedule" style={{ fontSize: '16px' }} />
                                    {new Date(entry.timestamp).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}
                                </span>
                            </div>
                            <div>
                                <h3 className="text-title truncate" style={{ fontSize: '1.2rem', margin: '0 0 8px 0', fontWeight: 600 }}>{entry.title}</h3>
                                <p className="text-body" style={{ fontSize: '0.95rem', color: 'var(--text-secondary)', WebkitLineClamp: 2, display: '-webkit-box', WebkitBoxOrient: 'vertical', overflow: 'hidden', margin: 0, lineHeight: 1.6 }}>
                                    {entry.content}
                                </p>
                            </div>
                        </div>
